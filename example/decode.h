...
SC_MODULE(decode) {
    ...
    ac_int < OPCODE_SIZE, false > opcode;
    ...
    ac_int < DCACHE_INDEX_WIDTH, false > last_ldst_index;
    ac_int < DCACHE_INDEX_WIDTH, false > last_ldst_index_temp;
    ...
    m_icount("m_icount"),
    o_icount("o_icount"),
    dec_stalls("dec_stalls"),
    total_cycles("total_cycles"),
    jump_stalls("jump_stalls"),
    ...
    void decode_th(void) {
        DECODE_RST: {
            ...
            dec_stalls.write(0);
            total_cycles.write(0);
            jump_stalls.write(0);
			branch_stalls.write(0);
			raw_stalls.write(0);
        ...
        DECODE_BODY: while (true) {
			...
			/*if ((jump && pc != self_feed.jump_address) || (branch && pc != self_feed.branch_address)) {
				imem_data = 0;
			}else {
			...
			if (freeze) {
				dec_stalls.write(dec_stalls.read() + 1);
			
			}
			total_cycles.write(total_cycles.read() + 1);
           ...
           if (insn == 0x0000006f) {
            ...
            case OPC_SYSTEM:
                ...
                case FUNCT3_EBREAK: // EBREAK, ECALL
                    ...
                    if (insn[20] == FUNCT7_EBREAK) { // Bit 20 discriminates b/n EBREAK and ECALL
                        ...
                        output.imm_u.set_slc(3, (ac_int<ZIMM_SIZE, false>) EBREAK_CAUSE); // force the exception cause on the zimm field
                        ...
                        #ifndef __SYNTHESIS__
                        debug_dout_t.imm_u.set_slc(3, (ac_int<ZIMM_SIZE, false>)ECALL_CAUSE);
                        #endif
                    }
                    break;
                case FUNCT3_CSRRW:
            ...
            if (output.alu_src == ALUSRC_RS2) {
                ...
                curr_temp_rs2.set_slc(0, (ac_int<12,false>)output.imm_u.slc<12>(8));
                if (output.imm_u[19] == 1) {
					curr_temp_rs2.set_slc(12, (ac_int<20,false>) 1048575);
				}
            } else if (output.alu_src == ALUSRC_IMM_S) {
				curr_temp_rs2.set_slc(0, (ac_int<5,false>)output.dest_reg);
				curr_temp_rs2.set_slc(5, (ac_int<7,false>)output.imm_u.slc<7>(13));
				...
				curr_temp_rs2.set_slc(12, (ac_int<20,false>) output.imm_u.slc<20>(0));
            }
            
            ac_int < XLEN, false > last_ldst_addr = (output.rs1 + curr_temp_rs2);
            last_ldst_index_temp = last_ldst_addr.slc<DCACHE_INDEX_WIDTH>(2 + DCACHE_OFFSET_WIDTH);
            ...
            if (load_instruction || (sen1_test && !forward_success_rs1) || (sen2_test && !forward_success_rs2)) {
				freeze = true;
				raw_stalls.write(raw_stalls.read() + 1);
			}  
            ...
            if (!freeze && output.regwrite[0] == 1 && output.dest_reg != 0) {
                sentinel[output.dest_reg].set_slc<XLEN>(1, pc); // Set corresponding sentinel flag.
